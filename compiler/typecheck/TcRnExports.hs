 ...
 = unsetWOptM Opt_WarnWarningsDeprecations $
   ...
   do   {
        ...
        ; reexp_module_ifaces_wtxts <- get_reexp_module_ifaces_wtxts
                                        (filter
                                          (\x ->
                                            fst x /= (moduleName this_mod))
                                          reexp_module_names_wtxts)

        ; let self_warn = maybe NoWarnings (WarnAll)
                            (lookup (moduleName this_mod)
                                    reexp_module_names_wtxts)
...
exports_from_avail (Just (L _ rdr_items)) rdr_env imports this_mod
  ...
  where
    ...
    exports_from_item acc@(ExportAccum ie_avails occs)
                      (L loc ie@(IEModuleContents _ _ (L lm mod)))
        | let earlier_mods
                = [ mod
                  | ((L _ (IEModuleContents _ _ (L _ mod))), _) <- ie_avails ]
    ...
    lookup_ie ie@(IEThingWith _ _ l wc sub_rdrs _)
        = do
